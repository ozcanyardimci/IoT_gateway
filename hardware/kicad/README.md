...
- **ioboard/** — power, relay outputs, digital inputs, analog I/O, RS485, RS232.
  ...
  lives entirely on this board. `power.kicad_sch`, `digital_inputs.kicad_sch`,
  `relay_outputs.kicad_sch`, `analog_io.kicad_sch`, and `rs485.kicad_sch` are captured;
  RS232 is next in the `docs/subsystems/` order.
- **lteboard/** — ESP32-S3 core compute, WiFi, LTE, Ethernet, status indication (I2C GPIO
  expander). `core-compute.kicad_sch` and `status_indication.kicad_sch` are captured; the
  rest of this board's subsystems follow the same pattern.
