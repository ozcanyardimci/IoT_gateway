...
## KiCad project structure (changed 2026-09-19)

`hardware/kicad/ioboard+lteboard/` is now the single active KiCad project — both boards'
10 subsystem sheets brought into one schematic hierarchy so steps 6/7 (pin planning,
board-to-board wiring) can be done with both boards visible at once instead of reasoned
about across two closed projects. `hardware/kicad/ioboard/` and `hardware/kicad/lteboard/`
are frozen as of this date — Ozcan's explicit decision — not edited further, kept only as
the historical record up to the merge point. Any subsystem file path referenced elsewhere
in this doc as living under `ioboard/` or `lteboard/` is now stale for editing purposes;
the current copy is under `ioboard+lteboard/`. Full merge detail, what's verified
connected, what's still pending, and the known open question about producing two PCB
files from one merged project at step 8/9: `docs/board-merge.md`.
...
- **Core compute (ESP32-S3)** — DONE, all 11 plan steps closed. Power/decoupling, EN reset,
  GPIO0 boot circuit, USB-C (data-only). Schematic: `hardware/kicad/ioboard+lteboard/
  ioboard+lteboard/core-compute.kicad_sch` (active copy since the 2026-09-19 merge — see
  "KiCad project structure" above). Full writeup: `docs/subsystems/core-compute.md`.
...
- **WiFi** — DONE, 2026-09-13. Small subsystem: the ESP32-S3-WROOM-1U's WiFi radio is
  already fixed (core-compute); all that was left was the external antenna path — J2 (SMA
  jack) added to `core-compute.kicad_sch`, module's U.FL → coax pigtail → J2, no PCB RF
  trace (confirmed against reference-hardware photos), shield to `EARTH`, RF pin No
  Connect. Verified at the coordinate level, not just a screenshot. Worth remembering:
  antenna gain must stay ≤2.33dBi to keep the module's existing FCC/CE certification valid
  without extra EMC testing — picked a 2dBi antenna for margin. Full writeup:
  `docs/subsystems/wifi.md`.
- **LTE** — DONE through schematic capture and BOM (2026-09-19). Largest, most complex
  subsystem: own rail, own UART through a level shifter (1.8V module <-> 3.3V MCU), SIM
  interface, PWRKEY/RESET power-control circuit, own antenna path (same coax-pigtail/SMA
  pattern as WiFi). Two findings surfaced while drafting it are already resolved (see
  `docs/subsystems/lte.md` decisions 2 and 8): (1) the rail — renamed `3V3_LTE` ->
  `VBAT_LTE` — had a divider set to ~3.3V, the exact floor of Quectel's 3.3-4.3V spec with
  zero margin against the 2-3A TX transient and below Quectel's own 3.8V typical; retapped
  U4's RFBB (137k -> 115k) to ~3.82V, with `power.md` updated to match (BOM, rail table,
  revision history); (2) `architecture.md`'s board-to-board header line now separately
  lists `VBAT_LTE` from `3V3_LOGIC`, ahead of roadmap step 7. Sheet symbol + 6
  hierarchical pins checked pin-by-pin against `lte.kicad_sch`'s own labels (full netlist
  reconstruction) 2026-09-19 — exact match. Two real gaps found in that pass (D1's 4th ESD
  channel unwired, two U4 no-connect flags missing) were fixed and re-verified clean. ERC
  and BOM's off-board items still pending, same as every other subsystem. Full writeup:
  `docs/subsystems/lte.md`.
...
- `hardware/kicad/README.md` — KiCad project layout
- `hardware/kicad/ioboard+lteboard/` — the active KiCad project (both boards, merged
  2026-09-19; see `docs/board-merge.md`)
- `hardware/kicad/ioboard/`, `hardware/kicad/lteboard/` — frozen as of 2026-09-19, kept
  for history only, not edited further
- `docs/board-merge.md` — the merge itself: what was done, what's verified connected,
  what's still pending, and the open PCB-layout question for steps 8/9
...
- **PCB layout from the merged project (flagged 2026-09-19, relevant at step 8/9):**
  `ioboard+lteboard` holds both physical boards' components in one netlist, and KiCad's
  "Update PCB from Schematic" has no per-sheet filter — it imports everything. Producing
  two clean per-board PCB files will need a deliberate approach then (most likely: two
  `.kicad_pcb` files that each import the full netlist and have the other board's
  footprints removed, redone after each schematic sync). Not blocking now — see
  `docs/board-merge.md` for the alternative that was considered and why it wasn't taken.
