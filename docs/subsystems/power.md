...
| 2026-09-23 | **Final project-wide ERC pass: PS1/U13 pin-type defect found and fixed.** Both isolated DC-DC modules (PS1, RK-0515S; U13, R1SX-3.33.3-R) are SnapEDA imports whose `-VOUT` pin was generically typed plain `Output` instead of the electrically-correct split (`+VOUT` = `Power output`, the genuinely driven pin; `-VOUT` = `Passive`, the return/reference side) — this reads to ERC as two driven outputs shorted together. Fixed on both symbols (library + embedded schematic copy). Also added 5 `PWR_FLAG`s this rail's nets needed since nothing on the merged `ioboard+lteboard` project drives them at the symbol level across sheet boundaries: `3V3_LOGIC`, `VBAT_LTE`, `5V_RELAY`, `GND_LOGIC`, `GND_ANALOG_ISO`. Project-wide ERC confirmed clean 2026-09-23. |
