...
| 2026-09-23 | **Final project-wide ERC pass.** PMODE0/1/2 (pins 45/44/43) showed as undriven inputs — genuinely floating by design (not a wiring bug), resolved with 10kΩ pull-ups to `3V3A`, the approach best supported after checking the datasheet, WIZnet's own reference schematics, and open-source W5500 designs (no single universal standard exists; picked with reasoning documented in-session, not a strong single source). `3V3A`/`AVDD` "not driven" traced to a missing `PWR_FLAG` on `3V3A` — added (also resolves `AVDD`, which is wired directly to `3V3A`, not a separate net). **C7's `1nF/2kW` typo (flagged 2026-09-13, never fixed) is now fixed** — corrected to `1nF/2kV` directly in `ethernet.kicad_sch`. `hardware/bom/ethernet_bom.csv` still not written — real reference designators have existed since Step 4, unblocked whenever it's picked up. Project-wide ERC confirmed clean 2026-09-23. |
